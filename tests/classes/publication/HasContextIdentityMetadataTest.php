...
class HasContextIdentityMetadataTest extends PKPTestCase
{
    ...
    // getStampableContextAbbreviation

    public function testStampableContextAbbreviationFallsBackToAcronymPerLocale(): void
    {
        $this->context->shouldReceive('getData')->with('abbreviation')->andReturn(['en' => 'J. X', 'de' => '']);
        $this->context->shouldReceive('getData')->with('acronym')->andReturn(['en' => 'JX', 'de' => 'ZX']);
        $this->assertSame(
            ['en' => 'J. X', 'de' => 'ZX'],
            Publication::getStampableContextAbbreviation($this->context)
        );
    }

    public function testStampableContextAbbreviationIsNullWhenNothingSet(): void
    {
        $this->context->shouldReceive('getData')->with('abbreviation')->andReturn(null);
        $this->context->shouldReceive('getData')->with('acronym')->andReturn(['en' => '']);
        $this->assertNull(Publication::getStampableContextAbbreviation($this->context));
    }
}
