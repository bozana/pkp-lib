...
abstract class ConvertLogFile extends \PKP\cliTool\CommandLineTool
{
    ...
    abstract public function getProcessingDir(): string;

    abstract public function getParseRegex(): string;

    abstract public function getPhpDateTimeFormat(): string;

    abstract public function getPathInfoDisabled(): bool;
    ...
    {
        $filePath = $this->getProcessingDir() . '/' . $fileName;
        ...
        $newFilePath = $this->getProcessingDir() . '/' . $path_parts['filename'] . '_new.log';
        if ($extension == 'gz') {
        ...
        if ($isSuccessful) {
            $renameFilePath = $this->getProcessingDir() . '/' . $path_parts['filename'] . '_old.log';
            ...
            case 'ojs2':
                $pageAndOp = $pageAndOp + [
                    Application::ASSOC_TYPE_SUBMISSION_FILE => [
                        'article/download', 'article/downloadSuppFile'],
    ...
    protected function setAssoc(int $assocType, string $op, array $args, array &$newEntry): void
    {
        switch ($assocType) {
            case Application::ASSOC_TYPE_SUBMISSION_FILE:
                ...
                // download/$submissionId/$representationId and
                // downloadSuppFile/$submissionId/$representationId, i.e. without $submissionFileId argument.
                // Newer log file versions will also not contain sumissionFileId for HTML files, when HTMLArticleGalley plugin is used.
                ...
                if (!$submissionFileId) { // Log fiels fro releases 2.x
            ...
            case Application::ASSOC_TYPE_SUBMISSION:
                ...
                $publicationId = $representationId = null;
                if (in_array('version', $args)) {
