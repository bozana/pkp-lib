...
abstract class PKPPubIdPlugin extends LazyLoadPlugin {
	 ...
	 * is used to mark a pub object for assigning the pub id to it.
	 * @return string
	 */
	...
	/**
	 * Should the pub id be assigned/generated for this object
	 * @param $pubObject object
	 * @return boolean
	 */
	function markedForAssignment($pubObject) {
		$assignFormFieldName = $this->getAssignFormFieldName();
		return $pubObject->getData($assignFormFieldName);
	}
	...
	function checkDuplicate($pubId, $pubObject, $contextId) {
		...
		// database.
		// TO DO: ???
		//$typesToCheck = array('Issue', 'Article', 'SubmissionFile');
