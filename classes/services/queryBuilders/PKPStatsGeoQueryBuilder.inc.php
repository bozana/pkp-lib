 ...
 *  metrics_counter_submission_geo_daily and metrics_counter_submission_geo_monthly table.
 */
...
class PKPStatsGeoQueryBuilder extends PKPStatsQueryBuilder
{
    ...
    /** @var array Include records for these countries */
    protected array $countries = [];

    /** @var array Include records for these regions */
    protected array $regions = [];

    /** @var array Include records for these cities */
     ...
     *
     * @return \PKP\services\queryBuilders\PKPStatsGeoQueryBuilder
     */
     ...
     * @return \PKP\services\queryBuilders\PKPStatsGeoQueryBuilder
     */
    public function filterByRegions($regions): self
    {
     ...
     * @return \PKP\services\queryBuilders\PKPStatsGeoQueryBuilder
     */
    public function filterByCities($cities): self
    {
    ...
    protected function _getObject(): \Illuminate\Database\Query\Builder
    {
        ...
        $q = DB::table('metrics_counter_submission_geo_monthly');
        ...
        if (!empty($this->countries)) {
            $q->whereIn(PKPStatisticsHelper::STATISTICS_DIMENSION_COUNTRY, $this->countries);
        }
        ...
        if (!empty($this->regions)) {
            ...
            $q->where(function ($q) use ($country, $region) {
                $q->where(PKPStatisticsHelper::STATISTICS_DIMENSION_COUNTRY, $country)
                    ->where(PKPStatisticsHelper::STATISTICS_DIMENSION_REGION, $region);
            });
        ...
        if (!empty($this->cities)) {
            ...
            $q->where(function ($q) use ($country, $region, $city) {
                $q->where(PKPStatisticsHelper::STATISTICS_DIMENSION_COUNTRY, $country)
                    ->where(PKPStatisticsHelper::STATISTICS_DIMENSION_REGION, $region)
                    ->where(PKPStatisticsHelper::STATISTICS_DIMENSION_CITY, $city);
            });
        ...
        $q->whereBetween(PKPStatisticsHelper::STATISTICS_DIMENSION_MONTH, [date_format(date_create($this->dateStart), 'Ym'), date_format(date_create($this->dateEnd), 'Ym')]);
