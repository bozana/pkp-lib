...
class PKPUsageEventLogListener
{
    ...
    /** @var FileCache $_regionsCache */
    public $_regionsCache;
    ...
    public function handle(UsageEvent $usageEvent)
    {
        ...
        $file = 'debug.txt';
        $current = file_get_contents($file);
        $current .= print_r("++++ usageEvent logged ++++\n", true);
        file_put_contents($file, $current);
    ...
    protected function prepareUsageEvent(UsageEvent $usageEvent): array
    {
        ...
        // Geo data
        // TO-DO: if Geo data tracking set up:
        ...
        // institutions IDs
        // TO-DO: if institution data tracking set up:
    ...
    */
    ...
    public function _getCachedIPLocation(string $ip, string $hashedIp, bool $flush): array
    {
        ...
        if (!isset($this->_regionsCache)) {
            $regionsCacheManager = CacheManager::getManager();
            $this->_regionsCache = $regionsCacheManager->getCache('regionsIsoCodes', 'all', [&$this, '_regionsCacheMiss']);
        }
        ...
        if (!array_key_exists($hashedIp, $cachedGeoData)) {
            ...
            try {
                ...
                if (!isset($regionIsoCode)) {
                    ...
                    $cachedRegions = $this->_regionsCache->getContents();
                    $foundRegions = array_filter($cachedRegions, function ($region) use ($countryIsoCode, $regionName) {
                        if (str_starts_with($region['code'], $countryIsoCode) && $region['name'] == $regionName) {
                            return $region;
                        }
                    });
                    // There should be only one region found (count($foundRegions) == 1)
                    $regionIsoCode = reset($foundRegions)['code'];
                }
    ...
    */
    ...
    /**
    * Region cache miss callback.
    */
    public function _regionsCacheMiss(\PKP\cache\FileCache $cache): array
    {
        $regionIsoCodesJson = file_get_contents('https://salsa.debian.org/iso-codes-team/iso-codes/-/raw/main/data/iso_3166-2.json');
        $regionIsoCodesArray = json_decode($regionIsoCodesJson, true);
        $cache->setEntireCache($regionIsoCodesArray['3166-2']);
        return $regionIsoCodesArray['3166-2'];
    }
}
