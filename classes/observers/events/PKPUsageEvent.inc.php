...
class PKPUsageEvent
{
    ...
    public $fileType;
    /** @var string $userAgent User UserAgent */
    public $userAgent;
    ...
    public function __construct(int $assocType, int $assocId, int $contextId, int $submissionId = null, int $representationId = null, string $mimetype = null)
    {
        ...
        switch ($assocType) {
            case Application::ASSOC_TYPE_SUBMISSION_FILE:
                ...
                $canonicalUrlParams = [$submissionId, $representationId, $assocId];
                break;
            case Application::ASSOC_TYPE_SUBMISSION:
                ...
                if ($op == 'view' && count($args) > 1) {
                    if ($args[1] == 'version' && count($args) == 3) {
                        ...
                        $canonicalUrlParams = [$submissionId, 'version', $publicationId];
                    }
                }
                break;
        ...
        if ($user) {
            ...
            foreach ([Application::CONTEXT_SITE, $contextId] as $workingContext) {
        ...
        */
        ...
        $current .= print_r("++++ PKP Usage Event ++++\n", true);
