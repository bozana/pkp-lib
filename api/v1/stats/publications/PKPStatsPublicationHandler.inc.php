...
AppLocale::requireComponents(LOCALE_COMPONENT_PKP_SUBMISSION, LOCALE_COMPONENT_APP_MANAGER, LOCALE_COMPONENT_PKP_MANAGER);
...
abstract class PKPStatsPublicationHandler extends APIHandler
{
    ...
    public function getManyCountries(\Slim\Http\Request $slimRequest, \PKP\core\APIResponse $response, array $args): \PKP\core\APIResponse
    {
        ...
        foreach ($totals as $total) {
            ...
            $columnNames = StatisticsHelper::getCounterMetricsColumns();
            $views = [];
            foreach ($columnNames as $columnName) {
                $views[StatisticsHelper::getReportMetricsColumnNames()[$columnName]] = $total->$columnName;
            }

            if ($responseCSV) {
                $items[] = $this->getGeoCSVItem($views, $countryName);
            } else {
                $items[] = $this->getGeoJSONItem($views, $countryName);
            }
        }
    ...
    public function getManyRegions(\Slim\Http\Request $slimRequest, \PKP\core\APIResponse $response, array $args): \PKP\core\APIResponse
    {
        ...
        foreach ($totals as $total) {
            ...
            if ($responseCSV) {
                $items[] = $this->getGeoCSVItem($views, $countryName, $regionName);
            } else {
                $items[] = $this->getGeoJSONItem($views, $countryName, $regionName);
            }
        }
    ...
    public function getManyCities(\Slim\Http\Request $slimRequest, \PKP\core\APIResponse $response, array $args): \PKP\core\APIResponse
    {
        ...
        foreach ($totals as $total) {
            ...
            if ($responseCSV) {
                $items[] = $this->getGeoCSVItem($views, $countryName, $regionName, $cityName);
            } else {
                $items[] = $this->getGeoJSONItem($views, $countryName, $regionName, $cityName);
            }
        }
    ...
    protected function _getGeoReportColumnNames(string $scale, bool $withPublication = false): array
    {
        ...
        $jsonViewsColumnNames = StatisticsHelper::getReportMetricsColumnNames();
        $reportViewsColumnNames = [];
        foreach ($jsonViewsColumnNames as $jsonViewsColumnName) {
            $reportViewsColumnNames[] = __("stats.{$jsonViewsColumnName}");
        }
        return array_merge(
            $publicationColumns,
            $scaleColumns,
     ...
     */
    protected function getGeoCSVItem(array $views, string $country, ?string $region = null, ?string $city = null): array
    {
        $item = [];
        if (isset($city)) {
            $item[] = $city;
        ...
        $reportViewsColumns = [];
        foreach ($jsonViewsColumnNames as $jsonViewsColumnName) {
            $reportViewsColumns[] = (int) $views[$jsonViewsColumnName];
        }
        return array_merge($item, [$country], $reportViewsColumns);
    }

    /**
     * Get JSON data with geographical (country, region, and/or city) metrics
     */
    protected function getGeoJSONItem(array $views, string $country, ?string $region = null, ?string $city = null): array
    {
        $item = [];
        if (isset($city)) {
            $item['city'] = $city;
        ...
        $reportViewsColumns = [];
        foreach ($jsonViewsColumnNames as $jsonViewsColumnName) {
            $reportViewsColumns[$jsonViewsColumnName] = (int) $views[$jsonViewsColumnName];
        }

        return array_merge($item, ['country' => $country], $reportViewsColumns);
    }
}
