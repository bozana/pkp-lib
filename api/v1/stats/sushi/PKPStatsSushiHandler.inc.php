...
class PKPStatsSushiHandler extends APIHandler
{
    ...
    {
        AppLocale::requireComponents(LOCALE_COMPONENT_PKP_SUSHI);
        AppLocale::requireComponents(LOCALE_COMPONENT_APP_SUSHI);
    ...
    public function getStatus(\Slim\Http\Request $slimRequest, \PKP\core\APIResponse $response, array $args): \PKP\core\APIResponse
    {
    ...
    public function getMembers(\Slim\Http\Request $slimRequest, \PKP\core\APIResponse $response, array $args): \PKP\core\APIResponse
    {
        $request = $this->getRequest();
        $context = $request->getContext();
        if (!$context) {
        ...
        $item = [
            ...
            'Name' => $institution->getLocalizedName(),
        ];
    ...
    public function getReports(\Slim\Http\Request $slimRequest, \PKP\core\APIResponse $response, array $args): \PKP\core\APIResponse
    {
        $request = $this->getRequest();
        $context = $request->getContext();
        if (!$context) {
    ...
    public function getReportsPR(\Slim\Http\Request $slimRequest, \PKP\core\APIResponse $response, array $args): \PKP\core\APIResponse
    {
        $args['report'] = new \APP\sushi\PR();
        return $this->getReport($slimRequest, $response, $args);
    }

    public function getReportsPR1(\Slim\Http\Request $slimRequest, \PKP\core\APIResponse $response, array $args): \PKP\core\APIResponse
    {
        $args['report'] = new \APP\sushi\PR_P1();
        return $this->getReport($slimRequest, $response, $args);
    }

    public function getReport(\Slim\Http\Request $slimRequest, \PKP\core\APIResponse $response, array $args): \PKP\core\APIResponse
    {
        $request = $this->getRequest();
        $context = $request->getContext();
        if (!$context) {
    ...
    public function processReportParams(&$report, $params): ?array
    {
        ...
        $warnings = [];
        // check dates
        // TO-DO: No Usage Available for Requested Dates ???
        ...
        // validate filters
        $filters = [
            ['Name' => 'Begin_Date', 'Value' => date_format(date_create($beginDate), 'Y-m-01')],
            ['Name' => 'End_Date', 'Value' => date_format(date_create($endDate), 'Y-m-t')],
        ];
        ...
        // if it is provided in the request and different that the context primary name.
